...
if sample == 'QCDInclusive2018':
	fileLists = {50: [], 100: [], 200: [], 300: [], 500: [], 700: [], 1000: [], 1500: [], 2000: []}
	SFs = {
				# QCD 2016
	...
	with open('samples/{}'.format(sampleFile)) as f:
		for i,line in enumerate(f):
			...
			for key in keys:
				...
				h.Scale(SF)
				fTemp.WriteObject(h,h.GetName())
			fileLists[HT].append(i + 1)
			fTemp.Write()
			fTemp.Close()
	print(fileLists)
	for HTBin in fileLists:
		fileList = []
		for f in fileLists[HTBin]:
			fileList.append('{}/{}-{}-temp.root'.format(outputDir, sample, f))
		os.system('hadd -f {}/{}-HT{}.root {}'.format(outputDir, sample, HTBin, ' '.join(fileList)))
	os.system('rm {}/*temp.root'.format(outputDir))
	'''
	fileList = []
	for j in range(i + 1):
