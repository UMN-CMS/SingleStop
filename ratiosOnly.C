void ratiosOnly(){
  ...
  if (doCompare){
    vector<string> compare = {"signal_1000_400","signal_1500_900","signal_2000_1900"};
    ...
    for(string n:plotNames){
	    ...
	    TH1D* plot_312 = new TH1D(); 
	    TH1D* plot_313 = new TH1D();
		...
		for (string inputDir:inputDirs) {
		    ...
		    if (strcmp(inputDir.c_str(), "313") == 0) {
		    	...
		    	plotTitle << s;
		    	plot_313->SetTitle(Form("%s_%s",plotTitle.str().c_str(), inputDir.c_str()));
		    	plot_313->SetLineWidth(1);
		    	plot_313->SetMarkerSize(0);
			plot_313->SetMarkerColor(kBlue);
		    	plot_313->Scale(1. / plot_313->Integral());
		    	//stack->Add(plot_313);
		    	xTitle = plot_313->GetXaxis()->GetTitle();
		    	yTitle = plot_313->GetYaxis()->GetTitle();
		    	//plot->Draw("HIST PLC PMC SAME");
		    	//if (plot->GetMaximum() > yMax) yMax = plot->GetMaximum();
		    }
	 	    else {
		 	plot_312 = (TH1D*)plots->Get(n.c_str());	
		     	stringstream plotTitle;
		    	//plotTitle << "(" << s.substr(0,s.find("_")) << ", " << s.substr(s.find("_") + 1) << ")";
		    	plotTitle << s;
		    	plot_312->SetTitle(Form("%s",plotTitle.str().c_str()));
		    	plot_312->SetLineWidth(1);
		    	plot_312->SetMarkerSize(0);
			plot_312->SetLineColor(kRed);
		    	plot_312->Scale(1. / plot_312->Integral());
		    	//stack->Add(plot_312);
		    	xTitle = plot_312->GetXaxis()->GetTitle();
		    	yTitle = plot_312->GetYaxis()->GetTitle();
		    	//plot->Draw("HIST PLC PMC SAME");
		    	//if (plot->GetMaximum() > yMax) yMax = plot->GetMaximum();
		    }
	    	}
		plot_312->Divide(plot_313);
		legend->AddEntry(plot_312);
		stack->Add(plot_312);	
	    } 
	    ...
	    //gPad->SetLogy();
	    c1->SaveAs(Form("plots/%s/overlaid/RatiosOnly/%s.pdf",saveTo.c_str(),n.c_str()));
    }
  }
